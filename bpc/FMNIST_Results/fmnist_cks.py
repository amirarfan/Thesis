...
for num_imgs in num_imgs:
    ...
    # %%
    x_convolved = convolve_filts(x_train, None, filters, bsize=32)

    print("Convolved")

    if precompute:
        np.save(f"x_convolved_{filters.shape[0]}.npy", x_convolved)
        np.save(f"filters_{filters.shape[0]}.npy", filters)
        sys.exit()
