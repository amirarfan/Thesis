...
print(x_train_moments.shape)
num_imgs = [2, 4, 20, 40, 80, 100, 120]


for num_img in num_imgs:
    images_for_filters = crs(x_train, x_train_moments, y_train, 5, num_img)
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "cv_accuracy_crshu.txt"), "a"
    ) as filename:
        filename.write(
